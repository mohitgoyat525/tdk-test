        ...
        <>
            ...
            <div className='container'>
                <nav className="flex items-center justify-between pt-11 pb-[22px] max-md:py-6">
                    <Link to="/" className='text-5xl font-normal lg:hidden'>
                        <img src="/assets/images/webp/logo.webp" alt="logo" className='w-full max-w-[75px] h-[17px]' />
                    </Link>
                    <div className={`z-40 flex gap-10 max-lg:flex-col max-lg:fixed max-lg:top-0 max-lg:w-full max-lg:h-screen max-lg:justify-center max-lg:items-center max-lg:bg-darkBlue max-lg:transition-all max-lg:duration-300 ${open ? 'bg-darkBlue max-lg:left-0' : 'left-full'}`}>
                        {NAVBAR_LIST.map((obj, i) => (
                            <Link
                                key={i}
                                to={obj.links} 
                                className="text-white text-[13px] font-normal relative after:absolute after:w-0 after:transition-all after:duration-300 after:hover:w-full after:bottom-0 after:h-0.5 after:bg-white pb-0.5 after:left-0"
                            >
                                {obj.name}
                            </Link>
                        ))}
                    ...
                    <div className='flex items-center flex-col lg:hidden'>
                        ...
                        >
                            ...
                            <span className={`w-6 transition-all duration-300 h-0.5 bg-white mb-1 rounded-md relative after:w-full after:h-full after:bg-white after:absolute after:top-0 after:left-0 after:rounded-md ${open ? 'rotate-45 after:rotate-90' : ''}`}></span>
